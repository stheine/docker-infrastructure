  ...
  return new Promise(resolve => {
    dns.query({
      ...
      try {
        ...
        logger.debug(`query failed (${server})`, checkErr.message);
...
const checkDnsServers = async function() {
  ...
  if(unavailableCount) {
    ...
    if(unavailableCount >= dnsServers.length - 1) {
      await mqttClient.publishAsync('dns/failed', `${unavailableCount} DNS-Server sind nicht erreichbar.`, {retain: true});

      logger.debug(`MQTT-Nachricht gesendet: ${unavailableCount} DNS-Server sind nicht erreichbar.`);
    }
  } else {
    await mqttClient.publishAsync('dns/failed', null, {retain: true});
  }
};
